...
（一个货真价实的阻塞调用），然后验证：
  ...
  6. 端点侧：`api/qa.py` 只 `await` 唯一入口 `qa_service.ask_with_sources(...)`，
     不再自行发起第二次检索（**源码断言** —— 该端点需要鉴权与完整 app，
     起 HTTP 端到端成本过高，而这一处的风险恰恰是「被人改回同步调用」或
     「改回先 ask() 再单独检索一遍」，源码断言能拦住这两个回归）。
...
def _stub_blocking(self, *args, **kwargs):
    ...
    【2026-09 合并修订】返回值必须是 `{answer, sources}`——这是 `_ask_blocking` 与
    `ask_with_sources()` 的真实契约（同步体自己做检索，所以 sources 由它一并给出）。
    替身的返回形状必须与被替换者一致，否则测的是替身而不是真代码。
    """
    time.sleep(BLOCK)
...
def main():
    ...
    # 契约：同步体自己完成检索，并把 {answer, sources} 一并返回（同一份数据 → 必然一致）
    check("_ask_blocking() 返回 {answer, sources}（与 ask_with_sources 的契约一致）",
          '"sources": sources' in body and "sources = self.search_related_nodes" in body)
    ...
    # 只断言**代码**：注释里为了说明历史写法会提到这些名字，若拿原文匹配，
    # 一句解释性注释就能让断言失效（这是源码断言的常见坑）。
    code = "\n".join(line.split("#", 1)[0] for line in src.splitlines())
    # 【2026-09 合并修订】原断言锁的是「端点内再 to_thread 检索一次」的写法。该写法
    # 会让同一次提问检索两遍（先 ask() 再单独 search_related_nodes()），而
    # CLAUDE.md「RAG 检索链路」明确禁止：sources 必须就是本次喂给 LLM 的上下文。
    # 现在端点只 await 唯一入口 ask_with_sources（其同步体内部已走 to_thread），
    # 故断言改为「入口唯一 + 端点不自行发起第二次检索」——保护意图不变，且更强。
    check("api/qa.py：qa_service.ask_with_sources 仍被 await",
          "await qa_service.ask_with_sources(" in code)
    check("api/qa.py：端点未自行发起第二次检索（检索只跑一次，见 CLAUDE.md）",
          "search_related_nodes" not in code)
    check("api/qa.py：同步链路未直接写在端点里（已下沉到 qa_service 的 to_thread）",
          "asyncio.to_thread" not in code)
